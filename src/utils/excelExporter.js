...
import { formatDateTimeBRT, nowBRTFormatted, todayBRTForFilename } from './dateUtils'

/** Formata data para exibição no fuso America/Sao_Paulo. */
const formatDateForExcel = (dateString) => {
  if (!dateString) return ''
  return formatDateTimeBRT(dateString)
}
...
export const exportFeedToExcel = (data, filters = {}, filename = null) => {
  try {
    ...
    const filterInfo = [
      ...
      [''],
      ['Data da Exportação:', nowBRTFormatted()],
    ...
    const fileName = filename || `tabulacoes_${todayBRTForFilename()}.xlsx`
